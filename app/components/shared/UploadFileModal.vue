...
<template>
  <Teleport to="body">
    ...
    >
      ...
      >
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-indigo-950/60 backdrop-blur-md dark:bg-slate-950/80" @click="$emit('close')"></div>
        
        <!-- Modal Content -->
        <div class="relative bg-white dark:bg-slate-900 rounded-t-3xl sm:rounded-[2.5rem] shadow-2xl max-w-3xl w-full overflow-hidden flex flex-col max-h-[95vh] border border-gray-100 dark:border-slate-800 transition-colors">
          <!-- Header -->
          <div class="p-6 sm:p-8 border-b border-gray-100 dark:border-slate-800 shrink-0 flex justify-between items-center bg-gray-50/50 dark:bg-slate-800/50">
            <div>
              <div class="flex items-center gap-3">
                <div class="p-2 bg-indigo-600 rounded-xl text-white shadow-lg shadow-indigo-200 dark:shadow-indigo-900/40">
                ...
                <h3 class="text-xl font-black text-gray-900 dark:text-white tracking-tight uppercase">Data Import</h3>
              </div>
              <p class="mt-1 text-xs text-gray-400 dark:text-slate-500 font-bold uppercase tracking-widest">Target: {{ type }}</p>
            </div>
            <button @click="$emit('close')" class="p-2 rounded-xl text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-slate-800 transition-all">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
          </div>

          <div class="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8 scrollbar-thin dark:scrollbar-thumb-slate-700">
            <!-- Format Guide -->
            <div v-if="stagedFiles.length === 0" class="space-y-4">
              <div class="flex items-center gap-2 text-indigo-800 dark:text-indigo-400 font-black text-[10px] uppercase tracking-[0.2em]">
              ...
              <div class="relative group">
                ...
                <pre class="relative text-[10px] bg-indigo-900 dark:bg-slate-950 p-6 rounded-2xl overflow-x-auto text-indigo-100 dark:text-indigo-400 font-mono shadow-xl border border-white/5">{{ JSON.stringify(exampleFormat, null, 2) }}</pre>
              </div>
            </div>
            ...
            <label
              ...
              class="flex flex-col items-center justify-center w-full h-72 border-2 border-dashed border-gray-100 dark:border-slate-800 rounded-[2rem] cursor-pointer bg-gray-50/50 dark:bg-slate-900/50 hover:bg-indigo-50/30 dark:hover:bg-indigo-900/10 hover:border-indigo-200 dark:hover:border-indigo-800 transition-all group relative overflow-hidden"
            >
              <div class="relative flex flex-col items-center justify-center pt-5 pb-6 text-center">
                <div class="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-xl shadow-indigo-100 dark:shadow-indigo-900/20 mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300">
                ...
                <p class="mb-2 text-base font-black text-gray-900 dark:text-white tracking-tight">
                  Drag & Drop JSON files
                </p>
                <p class="text-[10px] text-gray-400 dark:text-slate-500 font-black uppercase tracking-widest">Multiple files supported • Auto-detection</p>
              </div>
              <input
                ref="fileInput"
                type="file"
            ...
            <div v-else class="space-y-4">
              <div class="flex justify-between items-center mb-4">
                <h4 class="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-slate-500">Staged for Import ({{ stagedFiles.length }})</h4>
                <button @click="reset" class="text-[10px] font-black text-rose-500 hover:text-rose-700 uppercase tracking-widest px-3 py-1 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-900/30 transition-colors">Clear All</button>
              </div>
              
              <div v-for="file in stagedFiles" :key="file.id" class="group p-5 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-sm transition-all hover:shadow-xl hover:shadow-indigo-500/5 hover:border-indigo-100 dark:hover:border-indigo-900">
                <div class="flex justify-between items-start mb-6">
                  <div class="flex items-center gap-4 overflow-hidden">
                    <div class="bg-gray-100 dark:bg-slate-800 p-3 rounded-xl text-gray-400 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                    ...
                    <div class="overflow-hidden">
                      <p class="text-sm font-black text-gray-900 dark:text-white truncate">{{ file.filename }}</p>
                      <p class="text-[10px] font-bold text-emerald-600 dark:text-emerald-500 uppercase tracking-tight">{{ file.entries.length }} records identified</p>
                    </div>
                  </div>
                  <button @click="removeFile(file.id)" class="p-2 text-gray-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-xl transition-all">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <!-- Searchable Dropdown -->
                  <div class="relative">
                    <label class="block text-[10px] font-black uppercase text-gray-400 dark:text-slate-500 mb-2 tracking-widest ml-1">Target Language</label>
                    <div class="relative">
                      <button 
                        ...
                        :class="[file.detectedLocale ? 'border-indigo-100 dark:border-indigo-900 bg-indigo-50/30 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400' : 'border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-gray-400 dark:text-slate-600 hover:border-indigo-200']"
                      >
                      ...
                      >
                        <div v-if="file.isDropdownOpen" class="absolute left-0 right-0 top-full mt-2 z-50 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
                          ...
                          <div class="max-h-60 overflow-y-auto scrollbar-thin dark:scrollbar-thumb-slate-700">
                            <button 
                            ...
                            >
                              <span class="text-gray-700 dark:text-slate-300 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">{{ l.name }}</span>
                              <span class="text-gray-300 dark:text-slate-600 font-black group-hover:text-indigo-300">{{ l.code.toUpperCase() }}</span>
                            </button>
                  ...
                  <div v-if="file.error" class="flex flex-col justify-end">
                    <div class="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/50 rounded-2xl flex items-start gap-3 text-amber-700 dark:text-amber-400 transition-colors">
              ...
              <!-- Add More Button -->
              <label class="block p-6 border-2 border-dashed border-gray-100 dark:border-slate-800 rounded-3xl text-center cursor-pointer hover:bg-gray-50/50 dark:hover:bg-slate-800/50 hover:border-indigo-200 dark:hover:border-indigo-800 transition-all group">
                <span class="text-xs font-black uppercase tracking-widest text-gray-400 dark:text-slate-500 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">+ Add Additional Files</span>
          ...
          <!-- Footer -->
          <div class="bg-gray-50 dark:bg-slate-800/50 p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 border-t border-gray-100 dark:border-slate-800 shrink-0 transition-colors">
