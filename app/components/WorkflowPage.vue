...
const handleSyncAll = async () => {
  ...
  if (
    !confirm(
      `Are you sure you want to sync ALL ${readyLocales.length} locales to Strapi? This may take a while.`,
    )
  ) {
    return;
  }
  ...
  try {
    for (const locale of readyLocales) {
      ...
      if (result.failed > 0) {
        ui.addToast(
          `${locale.toUpperCase()}: Synced ${result.synced}, Failed ${result.failed}`,
          "info",
        );
      } else {
...
const handleUpdateEntry = (payload: {
  ...
  if (index !== -1) {
    ...
    store.setLocaleEntries(
      locale,
      entries,
      store.localeSource[locale] || "ai-generated",
    );
  }
};
...
const handleSyncConfirm = async () => {
  ...
  try {
    ...
    ui.addToast(
      `Successfully synced ${result.synced} entries to Strapi`,
      "success",
    );
    if (result.failed > 0) {
...
const hasLocaleData = computed(
  () => Object.keys(store.localeEntries).length > 0,
);
</script>
    ...
    >
      <div class="space-y-2">
        <div class="flex items-center gap-3">
          <NuxtLink
            to="/"
            class="p-2 bg-white rounded-xl border border-gray-100 shadow-sm text-gray-400 hover:text-indigo-600 transition-colors"
          >
            <svg
        ...
        <p
          class="text-base font-medium text-gray-500 max-w-2xl leading-relaxed"
        >
          {{ description }}
        </p>
      </div>
      <div
        v-if="store.isPulled"
        class="flex items-center gap-4 bg-white p-2 pl-4 rounded-2xl border border-gray-100 shadow-sm"
      >
        <div class="flex flex-col">
          ...
          <span class="text-sm font-bold text-gray-900"
            >{{ store.sourceEntries.length }} entries identified</span
          >
        </div>
    ...
    <div class="grid grid-cols-1 gap-8">
      ...
      >
        <div class="space-y-6">
          <div
            v-if="store.isPulled"
            class="p-4 bg-amber-50 border border-amber-100 rounded-2xl flex items-start gap-3 text-amber-800"
          >
            ...
            <div class="space-y-1">
              <p class="text-sm font-black uppercase tracking-tight">
                Warning: Session data exists
              </p>
              <p class="text-xs font-medium opacity-80">
                Re-generating will clear existing translations for this session.
                Make sure you have backed up any manual changes.
              </p>
            </div>
          </div>
          ...
          >
            <div class="flex items-center gap-4">
              ...
              >
                <div
                  class="relative flex items-center justify-center w-6 h-6 border-2 border-gray-200 rounded-lg transition-all group-hover:border-indigo-500 group-hover:scale-105"
                  :class="{
                ...
                <div class="flex flex-col">
                  <span
                    ...
                    >Unpublished Only</span
                  >
                  <span class="text-xs font-medium text-gray-400"
                    >Pull only untranslated entries</span
                  >
                </div>
              </label>
            </div>
            ...
            <div class="flex flex-wrap gap-3 w-full sm:w-auto">
              ...
              <button
                @click="handlePull"
                :disabled="isPulling"
                class="flex-1 sm:flex-none inline-flex items-center justify-center px-6 py-2.5 border border-transparent text-sm font-black rounded-xl shadow-lg shadow-indigo-200 text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-500/20 disabled:opacity-50 transition-all active:scale-95"
              >
                <svg
                  v-if="isPulling"
      ...
      >
        <div
          class="overflow-hidden rounded-2xl border border-gray-100 shadow-inner bg-gray-50/30"
        >
      ...
      >
        <div class="space-y-10">
          ...
          <div>
            <div class="flex items-center justify-between mb-6">
              <div class="space-y-1">
                ...
                <p class="text-xs font-medium text-gray-500">
                  Select at least one locale to proceed
                </p>
              </div>
              <div class="flex gap-4">
                <button
                  @click="selectAllLocales"
                  class="text-xs font-black uppercase tracking-widest text-indigo-600 hover:text-indigo-800 transition-colors"
                >
                  Select All
                </button>
                <button
                  @click="clearLocaleSelection"
            ...
            >
              <div v-for="l in locales" :key="l.code" class="relative group">
                <button
                ...
                >
                  <span class="truncate pr-2">{{ l.name }}</span>
                  <span
                    class="font-black opacity-30 text-[10px] tracking-tighter"
          ...
          <div
            v-if="
              isTranslating || Object.keys(store.localeJobStatus).length > 0
            "
            class="space-y-6 pt-10 border-t border-gray-50"
          >
            <div
              v-if="isTranslating"
              class="space-y-3 bg-indigo-50/50 p-6 rounded-3xl border border-indigo-100/50"
            >
              <div class="flex justify-between items-end">
                <div class="space-y-1">
                  <span
                    class="text-[10px] font-black uppercase tracking-widest text-indigo-400"
                    >AI Engine Working</span
                  >
                ...
                <span class="text-xl font-black text-indigo-600"
                  >{{
                    Math.round(
                      (store.translationProgress.current /
                        (store.translationProgress.total || 1)) *
                        100,
                    )
                  }}%</span
                >
          ...
          <div class="flex justify-end pt-8 border-t border-gray-50">
            <button
              ...
              class="inline-flex items-center px-8 py-3 border border-transparent text-sm font-black rounded-xl shadow-lg shadow-indigo-200 text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-500/20 disabled:opacity-50 transition-all active:scale-95"
            >
              <svg
                v-if="isTranslating"
      ...
      >
        ...
        >
          <div class="flex items-center gap-4">
            <div class="bg-gray-100 p-3 rounded-2xl text-gray-400">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
            ...
            <div class="space-y-1">
              ...
              <p class="text-xs font-medium text-gray-500">
                Use this to upload pre-translated JSON or correct AI errors.
              </p>
            </div>
          </div>
          <button
            @click="showUploadModal = true"
            class="w-full sm:w-auto inline-flex items-center justify-center px-6 py-2.5 border border-gray-200 shadow-sm text-sm font-black rounded-xl text-gray-700 bg-white hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:ring-4 focus:ring-gray-100 transition-all"
          >
            Upload JSON Files
          </button>
        </div>
        ...
        <div class="space-y-6">
          <div v-if="hasLocaleData" class="flex justify-end">
            <button
              ...
              class="inline-flex items-center px-6 py-2.5 border border-transparent text-sm font-black rounded-xl shadow-lg shadow-indigo-200 text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-500/20 disabled:opacity-50 transition-all active:scale-95"
            >
              <svg
                v-if="isSyncingAll"
                class="animate-spin -ml-1 mr-2 h-4 w-4 text-white"
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-100">
                ...
                <tbody class="bg-white divide-y divide-gray-50">
                  <tr
                    v-for="(entries, locale) in store.localeEntries"
                    :key="locale"
                  ...
                  >
                    <td class="px-6 py-5 whitespace-nowrap">
                      <div class="flex items-center gap-3">
                        <div
                          class="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-[10px] font-black text-gray-500 uppercase"
                        >
                          {{ locale.split("_")[0] }}
                        </div>
                        <div class="flex flex-col">
                    ...
                    <td class="px-6 py-5 whitespace-nowrap text-right">
                      <button
                        ...
                        class="group inline-flex items-center gap-2 text-sm font-black text-indigo-600 hover:text-indigo-800 transition-all ml-auto"
                      >
                        <template v-if="store.syncedLocales[locale]">
                          <div class="flex flex-col items-end">
                            ...
                            <span
                              ...
                              >{{
                                formatTime(store.syncedLocales[locale].at)
                              }}</span
                            >
                          </div>
                        </template>
                        <template v-else>
                          <span>Sync to Strapi</span>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="14"
                            height="14"
