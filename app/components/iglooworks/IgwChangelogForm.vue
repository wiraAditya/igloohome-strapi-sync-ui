...
const emit = defineEmits<{
  (e: 'save', data: { version: string, releaseDate: string, details: FormDetail[], locale: string }): void
  (e: 'cancel'): void
}>()
...
const fetchLocalizedDetails = async (locale: string) => {
  ...
  try {
    ...
    ui.addToast(`Failed to fetch ${locale.toUpperCase()} details: ${e.message}`, 'error')
  } finally {
...
onMounted(async () => {
  try {
    availableLocales.value = await strapi.getLocales()
  } catch (err: any) {
    ui.addToast(`Failed to load locales: ${err.message}`, 'error')
  }

  if (props.initialData) {
    // Create local string constants with guaranteed fallbacks to satisfy TS
    const version: string = props.initialData.version ?? ''
    const releaseDate: string = props.initialData.releaseDate ?? new Date().toISOString().split('T')[0]
    ...
    if (props.initialData.details && props.initialData.details.length > 0) {
      form.details = JSON.parse(JSON.stringify(props.initialData.details))
    }
...
const removeDetail = async (index: number) => {
  ...
  if (!detail.isNew && detail.id && selectedLocale.value !== 'en') {
    ui.addToast('Deletion of existing logs is restricted for non-English languages', 'warning')
    return
  }

  // Handle existing detail deletion (English only)
  if (!detail.isNew && detail.id) {
    if (!confirm('Are you sure you want to delete this log entry permanently? This cannot be undone.')) {
      return
    }
    try {
...
const handleSubmit = () => {
  ...
  const hasEmpty = form.details.some(d => !d.title.trim() || !d.content.trim())
  if (hasEmpty) {
  ...
  <div class="space-y-8 relative">
      ...
      <div class="space-y-6">
        ...
        >
          ...
          <button
            ...
            :disabled="!!(!detail.isNew && detail.id && selectedLocale !== 'en')"
    ...
    >
      <button
        type="button"
        @click="emit('cancel')"
        class="px-6 py-3 text-xs font-black uppercase tracking-widest text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200 transition-colors"
      >
        Cancel
      </button>
      <button
        type="button"
        @click="handleSubmit"
        class="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-black uppercase tracking-widest rounded-xl shadow-lg shadow-indigo-200 dark:shadow-none transition-all active:scale-95 flex items-center gap-2"
